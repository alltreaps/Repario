...
const generateAccessToken = (payload: AuthTokenPayload): string => {
...
const verifyToken = (token: string, secret: string): AuthTokenPayload | null => {
  try {
...
// ===============================
// Invoice Status + WhatsApp helpers
// ===============================

type InvoiceStatus = 'pending' | 'working' | 'done' | 'refused';

const VALID_STATUSES: InvoiceStatus[] = ['pending', 'working', 'done', 'refused'];

const DEFAULT_STATUS_MESSAGES: Record<InvoiceStatus, { default_message: string; allow_extra_note: boolean; send_whatsapp: boolean }> = {
  pending: {
...
async function ensureStatusSettingsForUser(userId: string) {
  ...
  if (existing && existing.length >= VALID_STATUSES.length) return existing;
...
async function getStatusSetting(userId: string, status: InvoiceStatus) {
...
async function sendWhatsAppMessage(phoneE164: string, message: string): Promise<{ success: boolean; details?: any }> {
...
const authenticateToken = asyncHandler(async (req: any, res: Response, next: NextFunction): Promise<void> => {
...
const requireAdmin = asyncHandler(async (req: any, res: Response, next: NextFunction): Promise<void> => {
  if (!req.user) {
...
app.get('/api/admin/users', authenticateToken, requireAdmin, async (req: any, res: Response): Promise<void> => {
  try {
...
app.put('/api/admin/users/:userId', authenticateToken, requireAdmin, async (req: any, res: Response): Promise<void> => {
  try {
...
app.post('/api/admin/users', authenticateToken, requireAdmin, async (req: any, res: Response): Promise<void> => {
  try {
...
app.patch('/api/admin/users/:userId/status', authenticateToken, requireAdmin, async (req: any, res: Response): Promise<void> => {
  try {
    const { userId } = req.params;
...
app.delete('/api/admin/users/:userId', authenticateToken, requireAdmin, async (req: any, res: Response): Promise<void> => {
  try {
    const { userId } = req.params;
...
if (process.env.NODE_ENV !== 'production') {
  app.post('/api/auth/dev-create-user', async (req: Request, res: Response): Promise<void> => {
    try {
...
// ===============================
app.get('/api/status-settings', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
...
app.put('/api/status-settings', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
    const settings = Array.isArray(req.body) ? req.body : [];
    const sanitized = settings
      .filter((s: any) => VALID_STATUSES.includes(s.status))
      .map((s: any) => ({
        user_id: userId,
        status: s.status,
...
app.post('/api/auth/register-business', async (req: Request, res: Response): Promise<void> => {
  try {
...
app.get('/api/layouts', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { data, error } = await supabase
      .from('layouts')
      .select(`
    ...
    const transformedData = data.map((layout: any) => ({
      id: layout.id,
      name: layout.name,
      ...
      sections: layout.layout_sections?.map((section: any) => ({
        id: section.id,
        title: section.title,
        sort_order: section.sort_order,
        fields: section.layout_fields?.map((field: any) => ({
          id: field.id,
          label: field.label,
          type: field.type,
          ...
          options: field.layout_field_options?.map((option: any) => ({
            id: option.id,
            label: option.label,
            value: option.value,
...
app.get('/api/layouts/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const transformedLayout = {
      ...
      sections: data.layout_sections?.map((section: any) => ({
        id: section.id,
        title: section.title,
        sort_order: section.sort_order,
        fields: section.layout_fields?.map((field: any) => ({
...
app.post('/api/layouts', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
...
app.post('/api/layouts/:id/sections', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const layoutId = req.params.id;
...
// Update section
app.patch('/api/layouts/:id/sections/:sectionId', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
...
// Delete section
app.delete('/api/layouts/:id/sections/:sectionId', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id: layoutId, sectionId } = req.params;
...
app.post('/api/layouts/:id/sections/:sectionId/fields', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id: layoutId, sectionId } = req.params;
...
// Update field
app.patch('/api/layouts/:id/sections/:sectionId/fields/:fieldId', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    let savedOptions: { id: string; label: string; value: string }[] = [];
...
// Delete field
app.delete('/api/layouts/:id/sections/:sectionId/fields/:fieldId', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id: layoutId, sectionId, fieldId } = req.params;
...
// Update layout
app.patch('/api/layouts/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const layoutId = req.params.id;
    ...
    if (defaultValue !== undefined) updateData.is_default = defaultValue;
...
// Delete layout
app.delete('/api/layouts/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const layoutId = req.params.id;
...
app.get('/api/invoices', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { data, error } = await supabase
      .from('invoices')
      .select(`
...
// ===============================
// INVOICE SYSTEM - FRESH START
// ===============================

// TypeScript interfaces matching frontend structure
...
const validateCustomerInfo = (customerInfo: any): customerInfo is CustomerInfo => {
  return (
    customerInfo &&
...
const validateInvoiceItem = (item: any): item is InvoiceItem => {
  return (
    item &&
    typeof item === 'object' &&
...
const validateInvoiceRequest = (data: any): data is InvoiceRequest => {
  return (
    data &&
    typeof data === 'object' &&
...
const findSimilarCustomers = async (customerName: string, userId: string, threshold: number = 0.85): Promise<any[]> => {
  try {
...
const findOrCreateCustomer = async (customerInfo: CustomerInfo & { id?: string }, userId: string, req: any): Promise<string> => {
  try {
    ...
    if (customerInfo.id) {
      ...
      const shouldUpdate = 
        (customerInfo.phone && customerInfo.phone !== existingCustomer.phone) ||
    ...
    const userSupabase = createClient(
      ...
      process.env.SUPABASE_ANON_KEY!, // Use anon key with user token
      {
        global: {
          headers: {
...
const verifyLayoutOwnership = async (layoutId: string | null, userId: string): Promise<boolean> => {
...
app.post('/api/invoices', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    const { customerInfo, layoutId, formData, items } = req.body as InvoiceRequest;
...
app.get('/api/invoices/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
...
app.put('/api/invoices/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
    const invoiceId = req.params.id;
...
app.get('/api/customers', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
...
app.post('/api/customers', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
...
app.get('/api/items', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { data, error } = await supabase
      .from('items')
      .select('*')
...
app.get('/api/items/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
// Create a new item
app.post('/api/items', async (req: any, res: Response): Promise<void> => {
  try {
    ...
    if (itemData.unit === '') itemData.unit = 'each';
...
app.patch('/api/items/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
app.delete('/api/items/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
app.put('/api/invoices/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    // Map frontend fields to DB columns
    if (req.body.customerId) updateData.customer_id = req.body.customerId;
    if (req.body.customer_id) updateData.customer_id = req.body.customer_id;
    if (req.body.layoutId) updateData.layout_id = req.body.layoutId;
    if (req.body.layout_id) updateData.layout_id = req.body.layout_id;
    if (req.body.formData) updateData.form_data = req.body.formData;
    if (req.body.form_data) updateData.form_data = req.body.form_data;
...
app.patch('/api/invoices/:id/status', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { status, extra_note } = req.body as { status: InvoiceStatus; extra_note?: string };
    const userId = req.user.userId;
...
app.delete('/api/invoices/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const userId = req.user.userId;
    const invoiceId = req.params.id;
...
// Update a customer
app.put('/api/customers/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
...
// Delete a customer
app.delete('/api/customers/:id', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    const customerId = req.params.id;
...
app.get('/api/customers/:customerId/history', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    invoices?.forEach(invoice => {
      ...
      const itemsTotal = invoice.invoice_items?.reduce((sum: number, item: any) => sum + (item.total || 0), 0) || 0;
      ...
      invoice.invoice_status_history?.forEach((statusChange: any) => {
        timeline.push({
          type: 'status_change',
          invoice_id: invoice.id,
    ...
    res.json({
      ...
      stats: {
        ...
        total_amount: invoices?.reduce((sum, i) => {
          const itemsTotal = i.invoice_items?.reduce((itemSum: number, item: any) => itemSum + (item.total || 0), 0) || 0;
